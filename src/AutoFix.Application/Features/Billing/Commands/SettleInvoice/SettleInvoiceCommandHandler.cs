...
{
    public class SettleInvoiceCommandHandler(ILogger<SettleInvoiceCommandHandler> logger,
    ...
    TimeProvider datetime) : IRequestHandler<SettleInvoiceCommand, Result<Success>>
    {
        ...
        {
            var invoice = await _context.Invoices.FirstOrDefaultAsync(w => w.Id == command.InvoiceId, ct);
